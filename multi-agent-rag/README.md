...
## RAG 문서 확장 (08-07/08-09) — 실제 스크래핑으로 서비스 커버리지 확대

**1차 시도(실수, 되돌림)**: `docs-aws-amazon-com-awsec2-latest-userguide-instance-types-html.md`가
"인스턴스 타입이 뭔지" 개념 설명만 있고 실제 t3.small 등 개별 스펙(vCPU/RAM)은 다른 페이지로 링크만
걸어둔 채 안 긁어와서, 관련 질문에 구체적으로 답을 못 하는 문제가 있었다(동료 테스터 피드백). 처음엔
스펙표/CPU 크레딧 메커니즘/워크로드별 사이징 가이드를 **직접 글로 써서** 문서로 넣었는데, 이는 실질적으로
"일반 지식 폴백을 미리 문서 파일 형태로 저장해둔 것"과 다르지 않아 RAG 본연의 "검증 가능한 외부 근거"라는
가치가 약하다는 지적을 받고 전량 삭제함.

**최종 방식**: `fetch_docs.py`(기존 12개 문서를 가져올 때 썼던 것과 동일한 스크래핑 파이프라인,
Playwright+trafilatura)로 아래 서비스의 **실제 AWS 공식 문서**를 새로 가져왔다:

- EC2 인스턴스 타입 실제 스펙표(`ec2/latest/instancetypes/gp.html` — t3.small의 실제 vCPU/RAM/네트워크
  대역폭 수치가 표로 들어있음, 이전 문제의 근본 해결)
- S3(개요 + 스토리지 클래스), RDS(개요 + DB 인스턴스 클래스 + Multi-AZ), DynamoDB(개요 + 용량 모드),
  VPC, CloudFront, Route 53, ECS, EKS

스크래핑 결과 중 실질적 내용이 거의 없는 페이지(예: DynamoDB "HowItWorks" 페이지가 "아래 섹션 참고"
한 줄만 있던 경우)는 노이즈로 판단해 제외했다. 문서 개수는 13개 → 26개로 늘었고 벡터DB 청크 수는
290개 → 773개. 인스턴스별 **$ 가격은 여전히 문서에 안 넣음** — 하드코딩된 가격표가 금방 stale해진다는
이유로 `pricing.py`(실시간 API)를 만들었던 설계 원칙과 충돌하기 때문에, 숫자는 계속 API가 전담하고
문서는 구조/메커니즘/설정 옵션만 담당하는 역할 분리를 유지함.
