...
질문 30개 = **검색만 필요(search_only) 15개** + **계산 필요(needs_calc) 15개**, 각 example에
...
30문항 확장판은 Anthropic API 사용량 한도로 실행이 중단되어(2026-08-01 00:00 UTC 리셋 이후 재실행 예정),
아래는 초기 10문항(검색 5 + 계산 5) 기준 예비 결과다.
...
30문항 결과로 재검증 필요 — 결과 나오는 대로 이 섹션을 갱신할 예정.
